...
export default function CustomCategories() {
  ...
  const [form, setForm] = useState({ name: "", keywords: "", naicsCodes: "", states: "", agencies: "", emailAlerts: true, smsAlerts: false })
  ...
  async function create() {
    ...
    try {
      ...
      setForm({ name: "", keywords: "", naicsCodes: "", states: "", agencies: "", emailAlerts: true, smsAlerts: false })
      await load()
  ...
  return (
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
      <div className="flex items-start gap-3 mb-5">
        ...
        <div><h2 className="text-white font-semibold">Personal Categories</h2><p className="text-sm text-slate-500 mt-1">Build your own bid feed using NAICS codes, exact keywords, states, and agencies. Alerts can follow each category.</p></div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <label className="text-xs text-slate-500">Agencies (optional)<Input className="mt-1.5 bg-slate-950 border-slate-700 text-white" placeholder="Forest Service, Army" value={form.agencies} onChange={e => setForm({ ...form, agencies: e.target.value })} /></label>
      </div>
      <label className="block text-xs text-slate-500 mt-3">Keywords / phrases<Textarea rows={3} className="mt-1.5 bg-slate-950 border-slate-700 text-white" placeholder="chiller repair, LED lighting, grounds maintenance" value={form.keywords} onChange={e => setForm({ ...form, keywords: e.target.value })} /></label>
